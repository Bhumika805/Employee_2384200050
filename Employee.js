...
try {
    employeePayrollData1.name = "John"; // Name should be capitalized as per regex
    console.log(employeePayrollData1.toString());
} catch (e) {
    console.error(e);
}
...
class Employee {
    constructor(name, id, wagePerHour, totalHours) {
        this.name = name;
        this.id = id;
    ...
    calculateWage() {
        return this.wagePerHour * this.totalHours;
    }
}
...
class EmployeePayroll {
    ...
    getTotalWages() {
        return this.employees.reduce((total, emp) => total + emp.calculateWage(), 0);
    }
}

// Example Usage
const payroll = new EmployeePayroll();
payroll.addEmployee(new Employee("John Doe", 1, 20, 40));
payroll.addEmployee(new Employee("Jane Doe", 2, 25, 35));

console.log("Total Wages: ", payroll.getTotalWages());
